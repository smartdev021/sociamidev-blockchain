...
const PopupAnswers = props => <div>These are popup answers</div>;
...
class AnswerQuestions extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.state.currentTask && this.state.currentTask.type == 'hangout') {
      ...
      this.state.currentTask.type == 'illuminate'
    ) {
      that.setState({ isQuestionsLoading: true });
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      prevState.isAnswersFetchFromCookiesInProgress !=
        this.state.isAnswersFetchFromCookiesInProgress ||
      prevState.isAnswersFetchFromServerInProgress !=
  ...
  fetchUserAnswersFromServerMy() {
    if (this.state.currentTask._id) {
      ...
      Axios.get(
        `${ConfigMain.getBackendURL()}/hangoutAnswerGetForTask?taskId=${
          this.state.currentTask._id
        }`
      )
        .then(response =>
          this.fetchUserAnswersFromServerMySuccess(response, that)
        )
        .catch(error => {
  ...
  getPartnerProfile() {
    ...
    const Partner = this.state.currentTask.metaData.participants.find(function(
      participant
    ) {
  ...
  fetchUserAnswersFromServerMySuccess(response, that) {
    ...
    const foundAnswersForPartner = answers.userAnswers.find(function(
      userAnswer
    ) {
  ...
  storeUserAnswersToCookies(answersMy) {
    if (this.state.currentTask._id) {
      ...
      let answersForTask = cookies.get(
        `answers_for_task_${this.state.currentTask._id}`
      );
      if (!answersForTask) {
        answersForTask = {};
      }
  ...
  fetchUserAnswersFromCookies() {
    if (this.state.currentTask._id) {
      ...
      const { cookies } = this.props;
      const answersForTask = cookies.get(
        `answers_for_task_${this.state.currentTask._id}`
      );
